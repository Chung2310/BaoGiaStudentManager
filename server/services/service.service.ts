...
export class ServiceService {
  ...
  static async getList(query: GetListQuery) {
    ...
    const list = await Service.find(filter).sort({ order: 1 }).skip(skip).limit(limit);
  ...
  static async seedDefaultData(defaultProjectId: any) {
    try {
      const count = await Service.countDocuments({ projectId: defaultProjectId });
      if (count === 0) {
        ...
        const defaultServices = [
          ...
          },
          {
            ...
            contents: {
              Basic: ["Hỗ trợ đưa dữ liệu/thao tác (Khóa học, lớp học, Học viên, Chương trình học, Hóa đơn, Hợp đồng) lần đầu sử dụng."],
              Plus: ["Hỗ trợ đưa dữ liệu/ thao tác (Khóa học, lớp học, Học viên, Chương trình học, Hóa đơn, Hợp đồng) trong 03 tháng đầu sử dụng"],
            },
            order: 2,
          },
          {
